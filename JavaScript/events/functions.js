...
{
    let time = new Date();
    if (document.getElementById("full-time"))
    {
        document.getElementById("full-time").innerHTML = time.toLocaleTimeString();
    }

    // 2. Управление анимацией хвоста
    let catArea = document.getElementById("cat-animation-area");
    let btnStart = document.getElementById("btn-start");

    // Если анимация существует И кнопка стоит на "Start" (т.е. таймер не активен)
    if (catArea && btnStart && btnStart.value === "Start")
    {
        catArea.style.animationPlayState = 'running';
    }
...
document.getElementById("btn-start").onclick = function startCountdownTimer()
{
    ...
    if (btnStart.value === "Start")
    {
        ...
        if (catArea) catArea.style.animationPlayState = 'paused';
    ...
    else
    {
        ...
        if (catArea) catArea.style.animationPlayState = 'running';
    }
}
